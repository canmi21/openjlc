...
pub fn get_temp_dir() -> PathBuf {
    ...
    Path::new(&home_dir).join(".canmi").join("openjlc").join("temp")
}
...
pub fn get_target_dir() -> PathBuf {
    let home_dir = env::var("HOME")
        .or_else(|_| env::var("USERPROFILE"))
        .unwrap_or_else(|_| ".".to_string());
    Path::new(&home_dir).join(".canmi").join("openjlc").join("target")
}
...
pub fn get_rule_dir() -> PathBuf {
    let home_dir = env::var("HOME")
        .or_else(|_| env::var("USERPROFILE"))
        .unwrap_or_else(|_| ".".to_string());
    Path::new(&home_dir).join(".canmi").join("openjlc").join("rule")
}
...
pub fn get_report_dir() -> PathBuf {
    let home_dir = env::var("HOME")
        .or_else(|_| env::var("USERPROFILE"))
        .unwrap_or_else(|_| ".".to_string());
    Path::new(&home_dir).join(".canmi").join("openjlc").join("report")
}
...
async fn download_rule_file(url: &str, dest_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    log::log(&format!("+ Downloaded rule file to '{}'", dest_path.display()));
    Ok(())
}
...
pub async fn check_and_download_rule_files() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !rule_dir.exists() {
        ...
        log::log(&format!("+ Created rule directory at '{}'", rule_dir.display()));
    } else {
        log::log(&format!("- Rule directory already exists at '{}'", rule_dir.display()));
    }
    ...
    if !altium_path.exists() {
        ...
        log::log(&format!("- Rule file already exists: '{}'", altium_path.display()));
    }
    ...
    if !kicad_path.exists() {
        ...
        log::log(&format!("- Rule file already exists: '{}'", kicad_path.display()));
    }
    ...
    if !report_dir.exists() {
        ...
        log::log(&format!("+ Created report directory at '{}'", report_dir.display()));
    } else {
