...
pub fn create_header_yaml(target_dir: &Path) -> io::Result<()> {
    ...
    let version = format!("v2.{}.{}.{}", rng.random_range(1..=2), rng.random_range(1..=37), rng.random_range(1..=3));
    let name = if rng.random_bool(0.5) { "EasyEDA" } else { "EasyEDA Pro" };
